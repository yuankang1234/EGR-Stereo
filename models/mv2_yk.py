...
class MobileNetV2(nn.Module):
    ...
    def __init__(self, num_classes=5, t=6):
        super(MobileNetV2, self).__init__()

        # 3 -> 32 groups=1 不是组卷积 单纯的卷积操作
        self.first_conv = Conv3x3BNReLU(3, 32, 1, groups=1)
        ...
        # 32 -> 64 stride=2 wh减半
        self.layer4 = self.make_layer(in_channels=32, out_channels=64, stride=1, factor=t, block_num=4)
        # 64 -> 96 stride=1 wh不变
    ...
    def forward(self, x):
        # 我总共下采样2次就够了,我希望卷积完是(1,320,64,128)
        x = self.first_conv(x)  # torch.Size([1, 32, 128, 256])#下采样一次
        x = self.layer1(x)  # torch.Size([1, 16, 128, 256])
        x = self.layer2(x)  # torch.Size([1, 24, 128, 256])
        x = self.layer3(x)  # torch.Size([1, 32, 128, 256])
        x = self.layer4(x)  # torch.Size([1, 64, 128, 256])
        # x = self.layer5(x)  # torch.Size([1, 96, 128, 256])
        # x = self.layer6(x)  # torch.Size([1, 160,64,128)#下采样一次
        # x = self.layer7(x)  # torch.Size([1, 320,64,128])
        return x
