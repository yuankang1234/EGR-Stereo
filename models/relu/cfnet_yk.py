...
from models.deformable_refine_1 import DeformableRefineF
from models.effnetv2 import effnetv2_ykdesign_3
...
class feature_extraction(nn.Module):
    def __init__(self, concat_feature=False, concat_feature_channel=12):
        ...
        self.channel_1_4 = 176
        effnetv2_stereo = effnetv2_ykdesign_3()
        self.inplanes = 32
        ...
        # efficientv2特征提取
        self.conv_stem = effnetv2_stereo.features[0]  # conv2d
        self.block1 = torch.nn.Sequential(*effnetv2_stereo.features[1:4])  # 1~3(3)
        self.block2 = torch.nn.Sequential(*effnetv2_stereo.features[4:9])  # 4~8(5)
        self.block3 = torch.nn.Sequential(*effnetv2_stereo.features[9:14])  # 9~13(5)
        self.block4 = torch.nn.Sequential(*effnetv2_stereo.features[14:26])  # 14~25(12)
        self.block5 = torch.nn.Sequential(*effnetv2_stereo.features[26:40])  # 26~39(14)
        self.block6 = torch.nn.Sequential(*effnetv2_stereo.features[40:56])  # 40~55(16)
        self.block7 = torch.nn.Sequential(*effnetv2_stereo.features[56:74])  # 56~73(18)

        self.gw2 = nn.Sequential(convbn(144, self.channel_1_4, 3, 1, 1, 1),
                                 Mish(),
                                 nn.Conv2d(self.channel_1_4, self.channel_1_4, kernel_size=1, padding=0, stride=1,
                                           bias=False))

        self.gw3 = nn.Sequential(convbn(176, self.channel_1_4, 3, 1, 1, 1),
                                 Mish(),
                                 nn.Conv2d(self.channel_1_4, self.channel_1_4, kernel_size=1, padding=0, stride=1,
                                           bias=False))

        self.gw4 = nn.Sequential(convbn(176, self.channel_1_4, 3, 1, 1, 1),
                                 Mish(),
                                 nn.Conv2d(self.channel_1_4, self.channel_1_4, kernel_size=1, padding=0, stride=1,
                                           bias=False))
        ...
        if self.concat_feature:
            ...
            self.concat2 = nn.Sequential(convbn(144, 128, 3, 1, 1, 1),
                                         Mish(),
                                         nn.Conv2d(128, concat_feature_channel, kernel_size=1, padding=0, stride=1,
                                                   bias=False))
            self.concat3 = nn.Sequential(convbn(176, 128, 3, 1, 1, 1),
                                         Mish(),
                                         nn.Conv2d(128, concat_feature_channel, kernel_size=1, padding=0, stride=1,
                                                   bias=False))

            self.concat4 = nn.Sequential(convbn(176, 128, 3, 1, 1, 1),
                                         Mish(),
                                         nn.Conv2d(128, concat_feature_channel, kernel_size=1, padding=0, stride=1,
                                                   bias=False))

        self.embedding_l3 = nn.Sequential(convbn(64, 64, kernel_size=3, stride=1, pad=1, dilation=1),
                                          nn.ReLU(inplace=True),
                                          nn.Conv2d(64, 64, kernel_size=1, padding=0, stride=1, bias=False))
        self.to_sf_l3 = StructureFeature()

        self.embedding_l4 = nn.Sequential(convbn(112, 64, kernel_size=3, stride=1, pad=1, dilation=1),
                                          nn.ReLU(inplace=True),
                                          nn.Conv2d(64, 64, kernel_size=1, padding=0, stride=1, bias=False))
        self.to_sf_l4 = StructureFeature()

        self.embedding_l5 = nn.Sequential(convbn(144, 64, kernel_size=3, stride=1, pad=1, dilation=1),
                                          nn.ReLU(inplace=True),
                                          nn.Conv2d(64, 64, kernel_size=1, padding=0, stride=1, bias=False))
        self.to_sf_l5 = StructureFeature()

        self.lastconv_1 = nn.Sequential(convbn(208, self.channel_1_4, 3, 1, 1, 1),
                                        nn.ReLU(inplace=True),
    ...
    def forward(self, x):
        ...
        l5 = self.block5(l4)  # (1,144,32,64) # 1/8
        l6 = self.block6(l5)  # (1,144,16,32) #1/16
        l7 = self.block7(l6)  # (1,176,16,32) #1/16
        ...
        featurecombine = self.lastconv_1(torch.cat((l3, l4, l3_sf, l4_sf), dim=1))  # (1,320,64,128)
        ...
        gw2 = self.gw2(l6)  # (1,320,32,64)
        gw3 = self.gw3(l7)  # (1,320,16,32)
        # gw4 = self.gw4(l8)  # (1,320,8,16) #我们取消掉1/32
        ...
        if not self.concat_feature:
            ...
            concat_feature2 = self.concat2(l6)
            concat_feature3 = self.concat3(l7)
            # concat_feature4 = self.concat4(l8)
            return {"gw1": gw1, "gw2": gw2, "gw3": gw3, "concat_feature1": concat_feature1,
...
class hourglassup(nn.Module):
    def __init__(self, in_channels):
        ...
        self.combine1 = nn.Sequential(convbn_3d(110, in_channels * 2, 3, 1, 1),
                                      Mish())
        self.combine2 = nn.Sequential(convbn_3d(174, in_channels * 4, 3, 1, 1),
                                      Mish())
        self.combine3 = nn.Sequential(convbn_3d(174, in_channels * 4, 3, 1, 1),
                                      Mish())
...
class CFNet(nn.Module):
    def __init__(self, maxdisp, use_concat_volume=False):
        ...
        self.change_176_32 = nn.Sequential(convbn(176, 128, 3, 1, 1, 1),
                                           nn.ReLU(inplace=True),
    ...
    def forward(self, left, right):
        ...
        left_fea_1 = self.change_176_32(features_left["gw1"])  # (1,32,64,128)
...
def CFNet_G_yk(d):
    return CFNet(d, use_concat_volume=False)
