...
from models.loss_entropy import model_loss
from utils import *
...
parser.add_argument('--datapath', default="/mnt/9c69d5cd-01cb-4603-b7c6-06d924734d0c/CYJ/dataset/scenflow_finalpass/",
                    help='data path')
parser.add_argument('--trainlist', default="./filenames_1/sceneflow_train.txt", help='training list')
parser.add_argument('--testlist', default="./filenames_1/sceneflow_test.txt", help='testing list')
...
def find_bottom_4(lst):
    ...
    # 返回前6个元素的值和下标
    bottom_4_values = [x[1] for x in sorted_lst[:10]]
    bottom_4_indices = [x[0] for x in sorted_lst[:10]]

    return bottom_4_values, bottom_4_indices

def train():
    # 记录开始训练的时间
    start_time_1 = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    loss_list = []
    D1_list = []
    EPE_list = []
    Thres1_list=[]
    ...
    for epoch_idx in range(start_epoch, args.epochs):
        ...
        if epoch_idx > 10:
            ...
            if nowerror < error:
                ...
                Thres1_list.append(avg_test_scalars["Thres1"][0])
                Thres2_list.append(avg_test_scalars["Thres2"][0])
                Thres3_list.append(avg_test_scalars["Thres3"][0])
        ...
        bottom_Thres1_values, bottom_Thres1_indices = find_bottom_4(Thres1_list)
        bottom_Thres2_values, bottom_Thres2_indices = find_bottom_4(Thres2_list)
        bottom_Thres3_values, bottom_Thres3_indices = find_bottom_4(Thres3_list)
        train_log_yk = open("train_log_scene.txt", mode="a", encoding="utf-8")
        ...
        print("Thres1前4个最小元素的值为：", bottom_Thres1_values, end="\t", file=train_log_yk)
        print("Thres1最小的下标为：", bottom_Thres1_indices, file=train_log_yk)
        print("Thres2前4个最小元素的值为：", bottom_Thres2_values, end="\t", file=train_log_yk)
        print("Thres2最小的下标为：", bottom_Thres2_indices, file=train_log_yk)
        print("Thres3前4个最小元素的值为：", bottom_Thres3_values, end="\t", file=train_log_yk)
        print("Thres3最小的下标为：", bottom_Thres3_indices, file=train_log_yk)
        # 记录结束训练的时间
